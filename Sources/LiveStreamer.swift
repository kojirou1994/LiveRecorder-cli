...
public final class LiveStreamer: JSONDeserializable {
	...
	public func record(force: Bool = false, autoRetry: Bool = false, location: String = "/Volumes/Sandisk_C10/Live_Recording") {
...
//		process?.launchPath = "/usr/local/bin/livestreamer"
//		process?.arguments = ["-v", "\(url)", "source", "-o", "\(location)/\(filename)"]
		...
		*/
		process?.terminationHandler = { proc in
			self.downloading = false
			print("Recording \(self.name) finished. Terminate code: \(self.process!.terminationStatus)")
			self.process = nil
		}
